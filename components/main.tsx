...
import { Sparkles, Camera, Send, Copy, Grid, LogOut } from 'lucide-react'
...
interface SocialMediaCaptionGeneratorProps {
  user: User
}

export default function SocialMediaCaptionGenerator({ user }: SocialMediaCaptionGeneratorProps) {
  ...
  return (
    <div className="min-h-screen relative bg-gradient-to-br from-gray-900 to-black text-white overflow-hidden">
      <div className="absolute top-6 left-10 z-10">
        ...
        <div className="mt-1 h-1 w-24 bg-gradient-to-r from-amber-400 via-orange-500 to-red-500 rounded-full"></div>
      </div>
      ...
      <div className="relative container mx-auto px-4 py-8 flex items-center justify-center min-h-screen">
        <Card className="w-full max-w-2xl bg-white/10 backdrop-blur-xl border border-white/20 shadow-2xl">
          ...
          <CardContent className="p-6 space-y-6">
            ...
            {response && (
              <div className="mt-6 rounded-xl p-4 bg-white/10 border border-white/20">
